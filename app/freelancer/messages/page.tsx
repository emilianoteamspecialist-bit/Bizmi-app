...
export default function FreelancerMessagesPage() {
  ...
  const fetchConversations = useCallback(async (userId: string) => {
    // Step 1: fetch raw conversations (no joins — bulletproof against FK naming)
    ...
    if (error) {
      // Dump everything we can — Supabase errors hide some props
    ...
    // Step 2: collect all participant IDs (including self so getParticipantProfile works)
    ...
    // Step 3: parallel-fetch profiles + avatars
    ...
    // Step 4: attach profile + avatar info to each conversation
    return convs.map((c) => ({
      ...c,
