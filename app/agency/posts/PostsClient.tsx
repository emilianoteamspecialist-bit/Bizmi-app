...
export default function PostsClient({
  ...
  // Initial jobs come from the server component as props (source-of-truth
  // pattern). Skip the first run of the search/load effect so we don't refetch
  // the same first page on mount; subsequent search changes still refetch.
  ...
  useEffect(() => {
    if (currentUserId) {
      // The first page (default search) was already fetched on the server and
      // seeded via props — skip that initial refetch. Any later change to the
      // debounced search term (or user) reloads from the client as before.
