...
export default function MessagesClient({ initialConversations, initialProfile }: MessagesClientProps) {
  const { user } = useAuth()
  const currentUserId = user?.id ?? null
  const [conversations, setConversations] = useState<any[]>(initialConversations)
  const [filteredConversations, setFilteredConversations] = useState<any[]>(initialConversations)
  ...
  const markMessagesAsRead = useCallback(
    async (conversationId: string, userId: string) => {
      ...
      if (error) {
        ...
        // Trigger a re-fetch of conversations to update unread counts in navbar
  ...
  // Initial conversations now come from the server component as props
  // (initialConversations) and seed state above. The per-conversation effect
  // below loads messages and marks them read for the seeded selection, and the
  // realtime subscriptions remain client-side.
  ...
  const uploadFile = async (file: File) => {
    ...
    const { error } = await supabase.storage.from("message-files").upload(filePath, file)
