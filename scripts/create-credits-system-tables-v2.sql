...
DROP FUNCTION IF EXISTS public.increment_user_credits(INTEGER, UUID);

-- Create function to increment user credits with correct parameter names
CREATE OR REPLACE FUNCTION increment_user_credits(uid UUID, amt INTEGER)
RETURNS void AS $$
BEGIN
    UPDATE public.profiles 
    SET credits = COALESCE(credits, 0) + amt,
        updated_at = NOW()
    WHERE id = uid;
END;
...
SELECT 'Credits system tables created successfully with correct function parameters!' as message;
