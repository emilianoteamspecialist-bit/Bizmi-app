-- Adds `has_applied` to get_jobs_with_details so the freelancer dashboard
-- doesn't need a follow-up query to compute it. The previous client-side
-- query could silent-fail (the destructure ignored the error), leaving every
-- job marked as not-applied even when the freelancer had already bid.
--
-- Drop-and-recreate is required because Postgres CREATE OR REPLACE on a
-- function rejects changes to the RETURNS row type. We also have to drop
-- all overloaded variants — `get_jobs_with_details(uuid, …)` has shipped in
-- a couple of arities historically.

DROP FUNCTION IF EXISTS get_jobs_with_details(UUID, TEXT, INTEGER, INTEGER, TEXT, TEXT, INTEGER, TEXT, TEXT[]);
DROP FUNCTION IF EXISTS get_jobs_with_details(UUID, TEXT, INTEGER, INTEGER, TEXT, TEXT, INTEGER, TEXT);
DROP FUNCTION IF EXISTS get_jobs_with_details(UUID, TEXT, INTEGER, INTEGER, TEXT, TEXT, INTEGER);
...
BEGIN
    ...
    WITH filtered_jobs AS (
        SELECT
            ...
            EXISTS (SELECT 1 FROM proposals p2 WHERE p2.job_id = j.id AND p2.freelancer_id = p_user_id) as applied,
