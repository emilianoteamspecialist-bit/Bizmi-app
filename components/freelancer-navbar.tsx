...
export default function FreelancerNavbar() {
  ...
  const getParticipantProfileForNotification = useCallback((message: any) => {
    ...
    if (senderProfile.account_type === "freelancer") {
      ...
      // For agency, set avatarUrl to empty to force fallback
      avatarUrl = ""
      displayName = senderProfile.company_name || senderProfile.full_name || "Agency"
      fallbackChar = (senderProfile.company_name || senderProfile.full_name)?.charAt(0) || "A"
      isFallback = true
    }
    return { full_name: displayName, avatar_url: avatarUrl, fallback_char: fallbackChar, is_fallback: isFallback }
  }, [])
  ...
  const loadUserData = useCallback(async () => {
    try {
      ...
      if (!user) {
        console.log("FreelancerNavbar: No user logged in, skipping data fetch.")
        setProfile(null)
        setLogoPreview("")
      ...
      console.log("FreelancerNavbar: Current User ID:", user.id)
      // Load profile data
      const { data: profileData, error: profileError } = await supabase
        ...
        .single()
      if (profileError) {
        console.error("FreelancerNavbar: Error fetching profile data:", profileError)
      } else if (profileData) {
        setProfile(profileData)
      }
      // Fetch freelancer logo for profile image
      const { data: logoData, error: logoError } = await supabase
        ...
        .single()
      if (logoError) {
        console.error("FreelancerNavbar: Error fetching freelancer logo:", logoError)
      } else if (logoData) {
      ...
      if (countError) {
        console.error("FreelancerNavbar: Error fetching total unread messages count:", countError)
      } else {
      ...
      if (recentMessagesError) {
        console.error("FreelancerNavbar: Error fetching recent unread messages for notifications:", recentMessagesError)
      } else {
        setRecentNotifications(recentUnreadMessages || [])
      }
    } catch (error) {
      console.error("FreelancerNavbar: Error loading user data:", error)
    }
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "freelancer_profile_updated") {
        console.log("FreelancerNavbar: Storage event detected, reloading user data.")
        loadUserData()
      }
    }
    window.addEventListener("storage", handleStorageChange)
    const handleCustomEvent = () => {
      console.log("FreelancerNavbar: Custom event detected, reloading user data.")
      loadUserData()
    }
    window.addEventListener("freelancer_profile_updated", handleCustomEvent)
    ...
    return () => {
      ...
      window.removeEventListener("freelancer_profile_updated", handleCustomEvent)
    }
    ...
    <nav className="sticky top-0 z-50 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="flex justify-between items-center h-14 sm:h-16">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-48">
                ...
                <DropdownMenuItem onClick={() => router.push("/freelancer/saved-jobs")}>
                  <Bookmark className="mr-2 h-4 w-4" />
                  Saved Jobs
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/freelancer/proposals")}>
                  <FileText className="mr-2 h-4 w-4" />
                  My Proposals
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => router.push("/freelancer/funded-jobs")}>
                ...
                <DropdownMenuItem onClick={() => router.push("/identity")}>
          ...
          {/* Right side - Dark Mode, Notifications (Messages Only), Profile */}
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-72 sm:w-80">
                    ...
                    recentNotifications.map((notification) => {
                      ...
                      return (
                        <DropdownMenuItem
                          key={notification.id}
                          onClick={() =>
                            router.push(`/freelancer/messages?conversationId=${notification.conversation_id}`)
                          }
                        ...
                        >
                          <Avatar className="h-8 w-8">
                            ...
                            <AvatarFallback
                              className={
                                sender?.is_fallback ? "bg-orange-500 text-white flex items-center justify-center" : ""
                              }
                            >
                              {sender.fallback_char || sender.full_name.charAt(0)}
                            </AvatarFallback>
                          </Avatar>
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => router.push("/freelancer/messages")} className="justify-center">
                  View All Messages
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-8 w-8 sm:h-10 sm:w-10 rounded-full">
                  <Avatar className="h-8 w-8 sm:h-10 sm:w-10">
                    <AvatarImage src={logoPreview || "/placeholder.svg?height=32&width=32"} alt="Profile" />
                    <AvatarFallback className="text-xs sm:text-sm">
                      {profile?.full_name?.charAt(0) || "JD"}
                    </AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
